...
namespace SmartDormitoryRepair.Api.Services
{
    public class TimeoutOrderChecker
    {
        ...
        public async Task CheckTimeoutOrders()
        {
            ...
            var timeoutTime = DateTime.Now.AddHours(-24);
            ...
            foreach (var order in timeoutOrders)
            {
                ...
                var notificationMessage = $"⚠️ 工单{order.Id}《{order.Title}》已超时24小时未处理！";
                ...
                Console.WriteLine($"⚠️ 超时工单: ID={order.Id}, 创建时间={order.CreateTime}");
            }
